...
class Orchestrator:
    ...
    async def _run_section_turn(self, session_id: str, prompt: str) -> None:
        """Send a section build or redirect prompt to OpenCode and handle errors.

        Scheduled as a fire-and-forget ``asyncio.Task`` by ``redirect_section``
        (N2-S12) and ``start_build_section`` (N2-S07).  Any exception is caught
        and converted into a ``turn.error`` bus event.

        No ``schema`` is passed (``schema=None``) — section turns use the file
        triplet as structured output, not OpenCode's native JSON schema
        mechanism (ADR-005).

        Args:
            session_id: The active OpenCode session ID.
            prompt: The section / redirect prompt text.

        N2-S12.
        """
        assert self._client is not None  # noqa: S101  # guarded by caller
        try:
            # schema=None — no structured output for section build/redirect (ADR-005).
